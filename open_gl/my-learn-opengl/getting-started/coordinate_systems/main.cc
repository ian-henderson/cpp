...
// settings
const unsigned int SCR_WIDTH = 1080 - 42;
const unsigned int SCR_HEIGHT = 1080 - 42;
...
int main()
{
    ...
    unsigned char *data = stbi_load(std::filesystem::absolute("resources/textures/container.jpg").c_str(), &width,
                                    &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(std::filesystem::absolute("resources/textures/awesomeface.png").c_str(), &width, &height,
                     &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.setMat4(
            "projection",
            projection); // note: currently we set the projection matrix each frame, but since the projection matrix
