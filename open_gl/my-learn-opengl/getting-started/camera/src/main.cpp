...
int main()
{
    ...
    // --------------------
    GLFWwindow *window =
        glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Camera", NULL, NULL);
    if (window == NULL)
    {
    ...
    // ------------------------------------
    Shader ourShader("resources/shaders/7.4.camera.vs",
    ...
    unsigned char *data = stbi_load(
        std::filesystem::absolute("resources/textures/container.jpg").c_str(),
    ...
    // load image, create texture and generate mipmaps
    data = stbi_load(
        std::filesystem::absolute("resources/textures/awesomeface.png").c_str(),
        &width, &height, &nrChannels, 0);
    if (data)
    {
